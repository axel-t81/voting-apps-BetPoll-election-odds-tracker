...
# Why the nested `polls/static/polls/` convention exists: 
# It's namespacing. 
# If you had multiple apps (`polls`, `blog`, `shop`) each with their own `style.css`, 
# the nested structure prevents filename collisions when 
# `collectstatic` gathers everything into one folder for production.

# Quick structure for your simple pottery bowl project**:
'''
betpoll/
├── static/              # Your project-wide static files
│   ├── css/
│   │   └── styles.css
│   └── js/
│       └── htmx.min.js  # Download HTMX here
├── staticfiles/         # Generated by collectstatic (add to .gitignore)
├── polls/
├── config/
└── manage.py
'''
